...
#[derive(Serialize)]
struct KeywordChangeRequestRow {
    request_id: String,
    request_type: String,
    group_type: String,
    base_keyword: String,
...
#[tauri::command]
fn get_stats(db_path: Option<String>) -> Result<AppStats, String> {
...
#[tauri::command]
fn get_companies(db_path: Option<String>, sort: Option<String>) -> Result<Vec<CompanySummary>, String> {
    let conn = open_db(db_path)?;
...
#[tauri::command]
fn get_keyword_tree(db_path: Option<String>) -> Result<Vec<KeywordGroupRow>, String> {
    let conn = open_db(db_path)?;
    ensure_keyword_group_type_column(&conn)?;
    let mut stmt = conn
        .prepare(
...
#[tauri::command]
fn list_site_requests(db_path: Option<String>) -> Result<Vec<SiteRequestRow>, String> {
    let conn = open_db(db_path)?;
    ensure_request_tables(&conn)?;
    let mut stmt = conn
        .prepare(
...
#[tauri::command]
fn list_site_health(db_path: Option<String>) -> Result<Vec<SiteHealthRow>, String> {
    let conn = open_db(db_path)?;
...
mod tests {
    ...
    #[test]
    fn db_path_candidates_include_project_root_data() {
        let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let project_root = manifest_dir.parent().expect("src-tauri has a parent");
        let expected = project_root.join("data").join("news_monitor.sqlite");

        assert!(
            default_db_path_candidates().iter().any(|path| path == &expected),
            "expected candidate was not present: {}",
            expected.display()
        );
    }
}
